...
def resolve_paper(version: str, build_id: int | None = None) -> dict[str, Any]:
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "PluginMatrix/0.3.0"},
    )
    try:
        with urllib.request.urlopen(request, timeout=30) as response:
            builds = json.load(response)
    except Exception as exc:
        raise PaperDownloadError(f"could not query Paper builds: {exc}") from exc
    stable = [build for build in builds if build.get("channel") == "STABLE"]
    candidates = stable or builds
    if not candidates:
        raise PaperDownloadError(f"no Paper builds found for {version}")
    if build_id is not None:
        matches = [item for item in candidates if item.get("id") == build_id]
        if not matches:
            raise PaperDownloadError(f"Paper build {build_id} was not found for {version}")
        build = matches[0]
    else:
        build = max(candidates, key=lambda item: item.get("id", 0))
    download = (build.get("downloads") or {}).get("server:default")
    if not download:
        raise PaperDownloadError(f"Paper build {build.get('id')} has no default server download")
    return {
...
def ensure_paper(version: str, cache_dir: Path, build_id: int | None = None) -> tuple[Path, dict[str, Any]]:
    ...
    if not jar_path.exists():
        request = urllib.request.Request(metadata["paper_download_url"], headers={"User-Agent": "PluginMatrix/0.3.0"})
        try:
            ...
            raise PaperDownloadError(f"could not download Paper: {exc}") from exc
    expected = metadata.get("paper_sha256")
    actual = hashlib.sha256(jar_path.read_bytes()).hexdigest()
    if expected and actual.lower() != expected.lower():
        jar_path.unlink(missing_ok=True)
        raise PaperDownloadError(f"Paper checksum mismatch: expected {expected}, got {actual}")
