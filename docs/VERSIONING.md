...
Unreleased work for the next minor version uses the PEP 440 suffix `.devN`. After an explicitly authorized Release Candidate Gate, a release candidate uses `MAJOR.MINOR.PATCHrcN`. The current source candidate is `0.8.0rc1`; the current public stable release remains `0.7.1`. Moving to a final `0.8.0` literal, creating a tag or publishing any package requires a separate owner-authorized release operation.
