...
Unreleased work for the next minor version uses the PEP 440 suffix `.devN`. After an explicitly authorized Release Candidate Gate, a release candidate uses `MAJOR.MINOR.PATCHrcN`. The current public stable release is `0.7.0`; moving to another final literal, creating a tag or publishing any package requires a separate owner-authorized release operation.
