...
Unreleased work for the next minor version uses the PEP 440 suffix `.devN`. After an explicitly authorized Release Candidate Gate, a release candidate uses `MAJOR.MINOR.PATCHrcN`. The current source and public stable version is `0.8.0`; future final-version changes, tags and package publication require separate owner authorization.
